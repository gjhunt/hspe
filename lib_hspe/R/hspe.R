...
#' @param loss_fn Loss function function minimized to find estimated proportions.
#'
#' (Optional) String.
#'
...
#' \code{resids_hat},\code{loss_hat}, and \code{p_hat} are the residuals, loss, and estimates for the proportions returned by hspe. Similarly, \code{resids_opt},\code{loss_opt} and \code{p_opt} are these values for the optimized value not re-scaled to enforce the STO constraint. 
#' }
#' }
#' @examples
#' \donttest{
...
#' n_markers = 20
#'
#' hspe(Y, pure_samples = pure_samples,
#' n_markers=n_markers)
#' }
#' 
#' @seealso \code{\link{find_markers}}
#' @export
hspe <- function(Y, references = NULL, pure_samples = NULL, n_markers = NULL, markers = NULL, 
    marker_method = "ratio", weights = NULL, sto = TRUE, inv_scale = function(x) 2^x, 
    fit_scale = log, loss_fn = "var", seed = NULL, verbose = FALSE, optim_opts = NULL) {
    ...
    est <- hspe_est_phats(Y = Y, pure_samples = pure_samples, markers = mrkrs, weights = weights, 
        optim_opts = optim_opts, sto = sto, inv_scale = inv_scale, fit_scale = fit_scale, 
        loss_fn = loss_fn, seed = seed, verbose = verbose)
    phats <- est$phats
...
hspe_est_phats <- function(Y, pure_samples = NULL, markers = NULL, weights = NULL, 
    optim_opts = NULL, sto = TRUE, inv_scale = function(x) 2^x, fit_scale = NULL, 
    loss_fn = "var", seed = NULL, verbose = FALSE) {
    ...
    dt_opt <- function(x) {
        ...
        popt <- opt$phat
        phat <- popt
        lossf <- function(p) L(p, x, Z, W, sto = FALSE, fit_scale = fit_scale, loss_fn = loss_fn)
        ...
        loss_hat <- L(phat, x, Z, W, sto = FALSE, fit_scale = fit_scale, loss_fn = loss_fn)
        i <<- i + 1
...
optim_solve <- function(x, Z, W = NULL, optim_opts = NULL, maxiter = 5000, tol = 1e-10, 
    p0 = NULL, fit_scale = NULL, loss_fn = "var", verbose = TRUE) {
    ...
    loss <- function(p) L(p, x, Z, W, sto = FALSE, fit_scale = fit_scale, loss_fn = loss_fn)
    if (optim_opts$constr == "eq_solve") {
...
optim_solve_nloptr <- function(x, Z, loss, loss_deriv, W = NULL, optim_opts = NULL, 
    maxiter = 5000, tol = 1e-12, p0 = NULL, verbose = TRUE) {
    ...
    out <- tryCatch({
        Kall <- length(p0)
        fs <- loss(p0)
        list(opt_out = nloptr::nloptr(x0 = p0, lb = rep(0, Kall), ub = rep(1, Kall), 
            eval_f = loss, opts = optim_opts), err = NULL)
    }, error = function(e) {
        list(opt_out = NULL, err = e)
    })
...
set_optim_opts <- function(optim_opts) {
    ...
    if (length(optim_opts) > 0 && optim_opts$pkg == "nloptr") 
        default <- list(pkg = "nloptr", constr = "box", algorithm = "NLOPT_GN_CRS2_LM", 
            maxeval = 5000, ftol_rel = 1e-12)
    for (nm in names(optim_opts)) {
        default[[nm]] <- optim_opts[[nm]]
    }
    if (default$pkg == "DEoptimR" && is.null(optim_opts$triter)) 
